...
void SDH::init(arma::mat td, arma::mat tl, int nb, int ni, double gl, double fl, double nu_, int na){
...
arma::mat SDH::gen_not_B(arma::mat td){

    int Ntest = td.n_rows;
    n_anchors = anchor.n_rows;
    arma::mat Phix = arma::zeros(td.n_rows, anchor.n_rows + 1);
    double si = 2.0* sigma* sigma;
    arma::mat xt = sum(square(td), 1);// 60000 * 1
...
//         std::cout << std::setprecision(3);                         // 等价于 cout <<setprecision(4) ;
        ...
        std::cout << "tl min: " << arma::min(tl);
        std::cout << "above is the wrong samples "<< std::endl;
        std::cout << "tl max: " << arma::max(tl);
