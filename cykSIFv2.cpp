...
cykSIF::cykSIF(){
    ...
    nn = 20;
    imgnum = 2000;
    lamda_final_train = "1,1,1,1,1,1,1,1;";
    train_scale = 80.0;
    idx_pool_size_ = 50;
...
void cykSIF::select_son_pool(int n_stage){ // select features for max correlation with trainlabel
    ...
    arma::mat idx_pool_t = idx_pool_.slice(n_stage);
    double ds = 0.7 * ((face(0,39)-face(0,45))*(face(0,39)-face(0,45)) + 
...
void cykSIF::pred_pic(cv::Mat& gray, arma::mat face, bool if_show){
    face_last_stage = face;
    for (int si = 0; si < n_stages_; ++si)
    {
        gen_all_features(si, gray, face);
        update_face(si);
    ...
    if (if_show)
    {
        ...
        draw_face(pano, face_current_stage, cv::Scalar(0,255,255));
        cv::imshow("pano", pano);
        cv::waitKey();
    }
}
