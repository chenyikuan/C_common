...
namespace cnpy {
	...
	template<typename T> std::vector<char> create_npy_header(const T* data, arma::umat shape, const unsigned int ndims);
	...
	void parse_npy_header(FILE* fp, unsigned int& word_size, arma::umat& shape, unsigned int& ndims, bool& fortran_order);
	void parse_zip_footer(FILE* fp, unsigned short& nrecs, unsigned int& global_header_size, unsigned int& global_header_offset);

	template<typename T> void npy_save(std::string fname, const T* data, arma::umat shape, std::string mode = "w") {
		const unsigned int ndims = shape.n_rows;
		FILE* fp = NULL;
...
class cyk_numpy_tools
{
public:
	template<typename T> void loadNPY(std::string fn, arma::Mat<T>& arma_data){
		raw_data = npy_load(fn);
		arma_data = arma::Mat<T>((T*)raw_data.data, raw_data.shape[1], raw_data.shape[0]);
		arma_data = arma_data.t();
	}
	template<typename T> void save2NPY(std::string fn, T& arma_data){
		raw_data.shape << arma_data.n_rows << arma_data.n_cols << endr;
		raw_data.shape = raw_data.shape.t();
		arma_data = arma_data.t();
		cnpy::npy_save(fn, arma_data.memptr(), raw_data.shape);
	}
protected:
private:
	cnpy::NpyArray raw_data;

	cnpy::npz_t npz_load(std::string fname);
	cnpy::NpyArray load_the_npy_file(FILE* fp);
	cnpy::NpyArray npz_load(std::string fname, std::string varname);
	cnpy::NpyArray npy_load(std::string fname);
