...
void cnpy::parse_npy_header(FILE* fp, unsigned int& word_size, arma::umat& shape, unsigned int& ndims, bool& fortran_order) {
	char buffer[256];
	...
	shape = arma::zeros<arma::umat>(ndims, 1);
...
cnpy::NpyArray cyk_numpy_tools::load_the_npy_file(FILE* fp) {
	arma::umat shape;
...
cnpy::npz_t cyk_numpy_tools::npz_load(std::string fname) {
...
cnpy::NpyArray cyk_numpy_tools::npz_load(std::string fname, std::string varname) {
	FILE* fp = fopen(fname.c_str(), "rb");
	...
	while (1) {
		...
		if (vname == varname) {
			cnpy::NpyArray array = load_the_npy_file(fp);
			fclose(fp);
			return array;
		}
		else {
...
cnpy::NpyArray cyk_numpy_tools::npy_load(std::string fname) {
	...
	cnpy::NpyArray arr = load_the_npy_file(fp);
